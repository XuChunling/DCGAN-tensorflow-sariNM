...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
         ...
         max_to_keep=1,
         input_fname_pattern='*.jpg', checkpoint_dir='ckpts', sample_dir='samples', out_dir='./out', data_dir='./data', lite_type=''):
    """
  ...
  def build_model(self):
    ...
    #if self.lite_type == 'D' or self.lite_type == 'G' or self.lite_type == 'S': # mod by xucl
    #  print("-----------> tflite not support image_summary,so no G_sum")
  ...
  def train(self, config):
    ...
    if config.G_img_sum:
      #if self.lite_type == 'D' or self.lite_type == 'G' or self.lite_type == 'S': # mod by xucl
      #  print("-----------> you choose create tflite, so no G_sum")
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      if not self.y_dim:
        ...
        return tf.nn.sigmoid(h4,name='d_lite'), h4
  ...
  def generator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      if not self.y_dim:
        ...
        return tf.nn.tanh(h4,name='g_lite')
  ...
  def fgraph_to_lite(self,lite_type="D"):
    ...
    #with tf.io.gfile.GFile('tflite/fgraph.pb', mode='wb') as f:
    #  f.write(fgraph.SerializeToString())
    ...
    if lite_type=="D":
      ...
      #    G_S_fgraph = tf.graph_util.convert_variables_to_constants(self.sess, self.sess.graph_def, output_node_names=["s_out"])
      #    with tf.io.gfile.GFile('tflite/G_S_graph.pb', mode='wb') as f:
      #      f.write(G_S_fgraph.SerializeToString())

      #    converter_S = tf.lite.TFLiteConverter.from_frozen_graph('tflite/G_S_fgraph.pb', [self.z], [self.sampler])
      #    S_model = converter_S.convert()
